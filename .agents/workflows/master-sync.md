...
2. Execute the `@.agents/skills/test-engineering/SKILL.md` skill.
3. **Strict Handoff:** If the test suite fails or escalates (Execution-Failure), `master-sync.md` MUST halt completely. Do not attempt to fix code in the Outer Loop. The agent must resolve the failure via the Inner Loop (`test-engineering` skill) and re-invoke `master-sync.md` only upon a clean test run.

## Phase 3: Update Documentation
1. Execute `@.agents/skills/doc-sync/SKILL.md`.
...
1. Execute `@.agents/skills/diagram-generation/SKILL.md`.
...
1. Execute `@.agents/skills/publish-showcase/SKILL.md`.
...
1. Ensure the commit message follows Conventional Commits (`feat:`, `fix:`, `docs:`, etc.).
2. The CI/CD pipeline in `.github/workflows/release.yml` will handle the version bump and changelog generation.
3. Semantic release rules are enforced by `.agents/rules/40-phase-deploy.md`.
...
3. Check `data/error_logs.json` to review any failures logged by the error observability hook.

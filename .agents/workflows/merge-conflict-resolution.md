...
- **`README.md` (Isolation Strategy):** DO NOT overwrite the target project's `README.md`. Keep the Base Environment documentation isolated in a separate file named `docs/antigravity/AGENT_DOCS.md`. Read the target project's `README.md` and append a single link to it (e.g., `[View Agentic Environment Documentation](docs/antigravity/AGENT_DOCS.md)`) at the bottom of the file. Ensure that credits to the original "study antigravity repo" are included in `docs/antigravity/AGENT_DOCS.md`.

...
2. Stage all changes, commit with a conventional commit message, and push to the remote origin:
   - `git add .`
   - `git commit -m "[User Message]"`
   - `git push`
3. If the host project includes a custom Git management script (e.g., `src/antigravity/capabilities/git_manager.py`), use that instead of raw git commands.

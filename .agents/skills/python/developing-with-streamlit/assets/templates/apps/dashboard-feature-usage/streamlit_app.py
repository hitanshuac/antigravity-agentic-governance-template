...
raw_data_section = st.expander("Raw data", expanded=False, icon=":material/table:", on_change="rerun")
if raw_data_section.open:
    with raw_data_section:
        display_df = filtered_data.copy()
        column_config = {}
        if normalize:
            column_config["request_count"] = st.column_config.NumberColumn("Requests", format="percent")
        st.dataframe(
            display_df,
            hide_index=True,

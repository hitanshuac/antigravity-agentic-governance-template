...
def generate_time_series(
    ...
    for category in categories:
        base = base_values.get(category, 1000) if base_values else rng.integers(500, 5000)
...
def dimension_metric(
    *,
    ...
    """
    chart_default = ":material/bar_chart: Bar" if default_chart == "Bar" else ":material/show_chart: Line"
    ...
    with st.container(border=True):
        ...
        with st.skeleton(height=CHART_HEIGHT):
            ...
            if "table" in (view_mode or ""):
                ...
                st.altair_chart(
                    create_bar_chart(filtered, "ds", y_col, dim_col, CHART_HEIGHT, show_percent),
                )
            else:
                st.altair_chart(

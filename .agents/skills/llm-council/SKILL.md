...
### DETERMINISTIC SAFEGUARD: Orchestrator Delegation
If the local repository contains a LangGraph Python orchestrator for the LLM council (e.g., `council_graph.py`), you MUST delegate the execution of steps 2-5 to that Python script rather than simulating it yourself. If no such script exists, proceed with the manual simulation below.
...
**DETERMINISTIC ARTIFACT RULE**: You MUST write the 5 advisor responses into a single scratch file named `scratch/council_advisors.md`. Do not just output them in your internal thoughts.
...
**DETERMINISTIC CHECKPOINT RULE**: You are FORBIDDEN from generating the final Chairman Synthesis until you have explicitly completed the peer review. You MUST write the peer review findings into a scratch file named `scratch/council_peer_reviews.md` before proceeding to Step 4.
...
### step 5: present the verdict in an artifact and HALT

After the chairman synthesis is complete, you MUST write the final verdict to an artifact named `council_verdict.md`.

**STOP HOOK (MANDATORY)**: After the `council_verdict.md` artifact is created, you MUST stop and ask the user: *"Would you like me to execute the recommended One Thing to Do First?"* Do NOT transition into Planning Mode or modify the codebase until the user explicitly approves the verdict.

Format the artifact as:

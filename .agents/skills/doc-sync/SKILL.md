...
description: "Update all markdown documentation (README, Handover, etc.) to strictly match the current state of the codebase, using a diff-based scan to avoid re-reading the entire tree on every run. TRIGGERS: 'update docs', 'sync documentation', 'refresh README', 'documentation is stale'."
---

# Documentation Sync

Ensure that all project documentation accurately reflects the current state of the codebase, without paying the token cost of re-reading everything every time.
...
1. Check for `.agents/architecture/.last_doc_sync` — a plain-text file containing the commit SHA of the last successful doc sync run.
2. If it exists, run:
   // turbo
   `git diff --name-only <last_sync_sha> HEAD`
3. If it does not exist (first run), treat this as a full scan — proceed to Phase 1, then write the current SHA to `.agents/architecture/.last_doc_sync` at the end of Phase 5.
4. **If the diff is empty:** halt immediately and report "No changes since last doc sync — skipping." Do NOT proceed.
5. **If the diff is non-empty:** only the changed paths feed into Phases 1–2 — not the full tree.
...
2. Identify new or modified public functions, classes, and API endpoints in those specific files.
...
Update `README.md` to reflect the latest state, editing only the sections affected by what changed in Phases 1–2. The README must contain:
...
- **Directory Structure**: Overview of `src/`, `data/`, `.agents/`.
...
1. Update `docs/antigravity/HANDOVER.md` only if a changed path touched a rule, workflow, or skill file.
2. Update `docs/antigravity/BOOTSTRAP.MD` only if a changed path added or removed a phase.
3. Do NOT update if no structural changes have occurred.
...
3. On approval, write the current `git rev-parse HEAD` to `.agents/architecture/.last_doc_sync`.

...
description: "Verify showcase assets, synthesize documentation, and prepare the README for publishing. TRIGGERS: 'publish showcase', 'prepare README', 'verify documentation assets', 'showcase diagrams'."
---

# Publish Showcase

Verify that all recruiter-facing and technical documentation assets are present, correctly referenced, and up to date. This skill does **NOT** handle git operations — those are delegated to `secure-checkpoint.md`.
...
1. **Recruiter-Facing PNG:** Confirm that `docs/assets/architecture_diagram_showcase.png` exists and is referenced at the top of `README.md` via `![Architecture Diagram](docs/assets/architecture_diagram_showcase.png)`.
   - If the PNG is **missing or stale**, execute `@.agents/skills/diagram-generation/SKILL.md` to regenerate all diagrams programmatically. Do NOT use an AI image generator directly outside of the strict rules.

...
1. **Bootstrap Workflow:** Execute the instructions outlined in `.agents/workflows/bootstrap.md`.
2. **Setup Git & Secrets:** Execute `.agents/workflows/setup-git.md` and `.agents/workflows/setup-secrets.md`.
3. **Scaffold Capabilities:** If building a Python project, copy the `src/antigravity/` directory from the template to get the pre-built capabilities (compaction, validation, database ingestion) *exactly* as defined in the governance rules.
4. **Verify:** Build the Eval Suite (`src/antigravity/tests/evals/`) to programmatically verify your capabilities before handing the project back to the human.
...
1. **Safe Merge:** Run `.agents/workflows/bootstrap.md` or apply `@.agents/skills/safe-merge/SKILL.md` to safely union-merge `.gitignore`, dependencies, and `.agents/` without deleting existing code. Wait for explicit manual approval before modifying any existing code.
2. **AST Dependency Discovery:** Use Repowise (`repowise init --yes --mode fast --no-editor-setup --no-claude-md --no-agents`) to build the Tree-sitter AST dependency graph across the legacy codebase.
3. **Refactor Incrementally:** Do not rewrite the whole app. Inject code quality standards and defensive programming from `.agents/skills/` into their existing LLM router cascade. Add Pydantic DLQ routing to their existing ingestion pipelines.
4. **Enforce Observability:** Log uncaught exceptions and errors structured in `data/error_logs.json` so you can diagnose and debug legacy issues deterministically.

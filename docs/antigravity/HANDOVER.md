...
3. **Scaffold:** Execute the instructions outlined in `docs/antigravity/BOOTSTRAP.MD`. If building a Python project, copy the `src/antigravity/` directory from the template to get the pre-built capabilities (compaction, validation, database ingestion) *exactly* as defined in the governance rules.
